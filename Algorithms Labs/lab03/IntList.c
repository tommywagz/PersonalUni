// COMP2521 Lab 01 Exercise - Linked Lists
//
// Pretend to be a human in text-based conversation.
//
// Authors:
// Tommy Wagner z5436162
//
// Written: 1/06/2023
...
void IntListInsertInOrder(IntList l, int v) {
	...
	struct IntListNode *current = head;
	...
	// If new int should be placed in the front:
	else if (v < head->data)
	{
		new->next = l->first;
		l->first = new;
...
IntList IntListSortedCopy(IntList l) {
	...
	while (current != NULL)
	{
		IntListInsertInOrder(newList, current->data);
		current = current->next;
	}
