...
void placeLocator(struct node *head, struct node *new, int value) {
	struct node *current = head;
	struct node *next = current->next;

	if (next == NULL && current->value <= value) {
		current->next = new;
	}
		...
		placeLocator(current->next, new, value);
	}
...
void listInsertOrdered(struct list *list, int value) {
	...
	// value goes in the beginning
	else if (value < list->head->value) {
		list->head = new;
		new->next = current;
	}

	// value goes at the end and list length is 1
	else if (next == NULL) {
		...
		placeLocator(current, new, value);
	}

	// locate where in the list the new node belongs

	if (listIsSorted(list) == true) {
		return;
	}
