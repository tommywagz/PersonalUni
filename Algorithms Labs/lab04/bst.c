...
int bstNumLeaves(struct node *t) {
	...
	return count + bstNumLeaves(t->left) + bstNumLeaves(t->right);
}

// Returns the range of the given BST
int bstRange(struct node *t) {
	// value of the right most leaf minus the value of the left most leaf
	...
	struct node *rightLeaf = t;
	struct node *leftLeaf = t;
...
struct node *bstDeleteLeaves(struct node *t) {
	...
	// locate each leaf and 
	// add their value to a queue

	// find each node with that value and 
	// delete it
...
void bstLevelOrder(struct node *t) {
	...
	QueueEnqueue(newQueue, t);

	while (QueueSize(newQueue) != 0) {
		struct node *front = QueueDequeue(newQueue);
		showBstNode(front);

		if (front->left != NULL) {
			QueueEnqueue(newQueue, front->left);
		}
		if (front->right != NULL) {
			QueueEnqueue(newQueue, front->right);
		}
	}
